...
- `app/components/HowItWorks.tsx` is the explanation of the method, shown as the second step of `WelcomeScreen` and reachable afterwards from Settings. It takes an optional `language` prop, which the wizard passes: it renders before the known language is stored, so reading it from the context would put the explanation in the default (German) for someone who has told the app nothing yet.
- **The known language is the first question of the first run**, ahead of the explanation, because it is the language everything after it is written in. Only that one picker is in English, and its rows carry the languages' own names (`NATIVE_LANGUAGE_NAME` in `copy.ts`) so there is nothing on it to read. The order is known language, explanation, learning language, level.
- `app/components/CoachMarks.tsx` is the walkthrough over the deck itself: it dims the screen and cuts a hole around one region at a time. `HomeScreen` measures the regions, because only it knows which views they are, and a step whose measurement did not arrive passes `rect: null` and shows its caption over a plain dim. It runs once, on the first deck a learner opens (`homeTourSeen`), and Settings can arm it again - `app/services/tourSignal.ts` is how the Settings tab tells the still-mounted Home tab, the same shape as `deckSignal` and for the same reason. The card is wrapped in a plain `View` purely so it can be measured: `measureInWindow` reports where a transform has put a view, and the card carries both the drag transform and the mount scale.
